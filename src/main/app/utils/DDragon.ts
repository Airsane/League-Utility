...
export class DDragon {
  ...
  private async getRunesReforged(): Promise<RuneReforged[]> {
    if (!this.latestVersion)
      this.latestVersion = await this.getLatestVersion();
    console.log(`${this.URL}cdn/${this.latestVersion}/data/en_US/runesReforged.json`);
    const {data} = (await axios.get(`${this.URL}cdn/${this.latestVersion}/data/en_US/runesReforged.json`));
    return data;
  }
