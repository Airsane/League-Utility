...
export default class Logger {
  ...
  constructor(directory:string = __dirname,filename: string = "latest.log") {
    this.filename = filename;
    this.directory = directory;
    this.filePath = path.join(this.directory,"log/" + this.filename);
    this.generateFolder();
    this.generateLogFile();

  }

  private generateFolder(): void {
    const dir = path.join(this.directory, "log/");
  ...
  private generateLogFile(): void {
    if(fs.existsSync(this.filePath)){
      ...
      fs.renameSync(this.filePath,path.join(this.directory, "log/" + `${birthtime.getFullYear()}${("0" + (birthtime.getMonth() + 1)).slice(-2)}${("0" + (birthtime.getDate())).slice(-2)}_${("0" + birthtime.getHours() ).slice(-2)}${("0" + birthtime.getMinutes()).slice(-2)}${("0" + birthtime.getSeconds()).slice(-2)}.log`));
    }
    const date = new Date();
